...
**Edge Cases:** Simultaneous acceptances of the same Offer — resolved by **first eligible acceptance by trusted server timestamp**. **SUPERSEDED 2026-08-22:** the former *"price-time priority (best rate first, then oldest listing first)"* rule is withdrawn — best-rate and price-time **allocation** priority are not used, and no central-limit-order-book semantics apply. Marketplace discovery/ranking may still order listings by rate or time; ranking a listing is not allocating it. **Tie-break — deterministic, added 2026-08-24.** Where two eligible acceptances carry the same trusted server timestamp at stored precision, priority is resolved by a unique server-generated ordering key assigned within the same acceptance serialization boundary, giving the total order `(accepted_at ASC, server_order_key ASC)`. The order is server-authoritative, deterministic and replayable for audit; the ordering key is a **required, immutable property of the accepted allocation** and its durable persistence is **required** of the later domain-model implementation, which Phase 1 does not yet provide; the seller's rate is never a priority mechanism. The persistence mechanism for the ordering key is implementation-dependent (`02_Technical_Design_Specification.md`, marketplace semantics).
