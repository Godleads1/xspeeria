...
| Non-ledger **persisted transactional** amounts | **Integer minor units — `amount_minor BIGINT` + `currency` + `scale` + `currency_def_version`. SUPERSEDED 2026-08-25 by DECISION S4-1 (HUMAN-APPROVED).** The previous position — *"Exact `Decimal` / `NUMERIC(20,4)` per TDS §6.4 — quoted amounts, offers, requests"* — is **withdrawn for authoritative transactional monetary state** (`Offer`, `Match`, `Transaction`, `Settlement`, `SettlementLeg`, `PayoutExecution`). One representation now governs every persisted authoritative monetary amount, ledger and non-ledger alike, so no conversion boundary sits inside a money-path transaction. `NUMERIC`/`DECIMAL` may exist **only** as an explicitly derived, non-authoritative presentation/reporting value with a justified need, and must never be read back as truth. **Exchange rates are not monetary amounts and are out of scope of this row** — `desired_rate` / `agreed_rate` remain `NUMERIC(12,6)`, unchanged |
